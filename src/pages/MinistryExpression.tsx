...
import {
  ...
  ArrowRight, BookOpen, Sparkles, Clock, Heart
} from 'lucide-react'
...
function SectionLabel({ text, dark = true }: { text: string; dark?: boolean }) {
  return (
    <div className="flex items-center gap-4 mb-4">
      <div className={`w-8 h-px ${dark ? 'bg-[#c9a84c]' : 'bg-[#c9a84c]'}`} />
  ...
  return (
    <>
      ...
      <section className="relative bg-gradient-to-br from-[#050e1d] via-[#0a1628] to-[#0f2040] pt-40 pb-28 overflow-hidden">
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 font-serif font-bold text-white/[0.025] pointer-events-none select-none leading-none" style={{ fontSize: '18rem' }}>M</div>
        ...
        <div className="max-w-6xl mx-auto px-8 lg:px-12 relative z-10">
          ...
          <motion.h1
            ...
            Ministry<br />
            <span style={{ background: 'linear-gradient(135deg,#e8c97a,#c9a84c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Expression</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="text-white/55 text-base max-w-2xl leading-[1.85]">
            Four distinct channels through which this ministry serves the Body of Christ, each designed
            to meet believers at a specific point of need and carry them into greater depth, clarity, and
            effectiveness in God.
          </motion.p>

          {/* Jump links — correct order */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
            className="flex flex-wrap gap-2 mt-10">
            {[
              { label: 'Itinerant Ministry', icon: Globe, id: 'itinerant', num: '01' },
              { label: 'Prayer Lab', icon: Flame, id: 'prayer-lab', num: '02' },
              { label: 'Mentorship Table', icon: Users, id: 'mentorship', num: '03' },
              { label: 'Music', icon: Music, id: 'music', num: '04', soon: true },
            ].map(({ label, icon: Icon, id, num, soon }) => (
              <button key={id} onClick={() => scrollTo(id)}
                ...
                <span className="text-[9px] text-white/30">{num}</span>
                <Icon size={11} />
                {label}
      ...
      {/* ══════════════════════════════════════════════════════════
          01. ITINERANT MINISTRY — BLUE (navy)
      ══════════════════════════════════════════════════════════ */}
      <section id="itinerant" className="bg-[#f8f6f0] scroll-mt-20">
        <div className="max-w-6xl mx-auto px-8 lg:px-12 py-24">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 border border-[#0a1628]/25 flex items-center justify-center text-[#0a1628]"><Globe size={18} /></div>
            <div>
              <span className="text-[9px] tracking-[0.3em] uppercase text-[#0a1628]/40 font-medium block">01</span>
              <span className="text-[9px] tracking-[0.2em] uppercase text-[#c9a84c] font-semibold">Across Places and Platforms</span>
            </div>
          </div>
          <h2 className="font-serif font-semibold text-[#0a1628] leading-[1.1] mb-2" style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)' }}>
            Itinerant Ministry
          </h2>
          <p className="font-serif italic text-[#0a1628]/45 text-lg mb-12">
          ...
          <div className="grid lg:grid-cols-2 gap-16">
              ...
              <div className="grid grid-cols-2 gap-2">
                {['Churches', 'Campuses', 'Conferences', 'Fellowships', 'Special Gatherings', 'Online Platforms'].map((v, i) => (
                  <motion.div key={v}
                    ...
                    <span className="text-sm text-[#444] group-hover:text-[#0a1628]">{v}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="bg-[#0a1628] px-8 py-10 mb-6 relative overflow-hidden">
      ...
      {/* ══════════════════════════════════════════════════════════
          02. PRAYER LAB — GOLD / dark navy
      ══════════════════════════════════════════════════════════ */}
      <section id="prayer-lab" className="bg-[#060d1a] scroll-mt-20">
        <div className="max-w-6xl mx-auto px-8 lg:px-12 py-24">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 border border-[#c9a84c]/30 flex items-center justify-center text-[#c9a84c]"><Flame size={18} /></div>
            <div>
              <span className="text-[9px] tracking-[0.3em] uppercase text-[#c9a84c]/60 font-medium block">02</span>
              <span className="text-[9px] tracking-[0.2em] uppercase text-[#c9a84c] font-semibold">Monthly Meeting</span>
            </div>
          </div>
          <h2 className="font-serif font-semibold text-white leading-[1.1] mb-2" style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)' }}>
            Prayer Lab
          </h2>
          <p className="font-serif italic text-white/45 text-lg mb-12">
                ...
                <div className="space-y-1 mb-10">
                  {prayerLabExpects.map(({ icon: Icon, text }, i) => (
                    <motion.div key={text}
                      ...
                      <p className="text-white/70 text-sm leading-snug group-hover:text-white transition-colors duration-300">{text}</p>
                    </motion.div>
                  ))}
                </div>
      ...
      {/* ══════════════════════════════════════════════════════════
          03. MENTORSHIP TABLE — BLUE (light bg)
      ══════════════════════════════════════════════════════════ */}
      <section id="mentorship" className="bg-[#f8f6f0] scroll-mt-20">
        <div className="max-w-6xl mx-auto px-8 lg:px-12 py-24">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 border border-[#0a1628]/25 flex items-center justify-center text-[#0a1628]"><Users size={18} /></div>
            <div>
              <span className="text-[9px] tracking-[0.3em] uppercase text-[#0a1628]/40 font-medium block">03</span>
          ...
          <div className="grid lg:grid-cols-5 gap-16">
            <motion.div className="lg:col-span-3" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['Spiritual Understanding', 'Personal Discipline', 'Purpose and Assignment', 'Leadership', 'Communication', 'Life Development', 'Faith and Character'].map(area => (
                    <span key={area} className="text-[10px] px-3 py-1.5 border border-[#c9a84c]/25 text-[#444] hover:border-[#c9a84c] hover:text-[#0a1628] transition-all duration-300 tracking-wide">
                      {area}
                    </span>
                  ))}
                </div>
            ...
            <motion.div className="lg:col-span-2" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="relative mb-8 pl-5 border-l-2 border-[#c9a84c]">
                <p className="font-serif italic text-[#0a1628] text-xl leading-[1.6]">
              ...
              <div className="space-y-1 mb-8">
                {['Sharpened', 'Strengthened', 'Equipped', 'Grounded', 'Effective', 'Intentional'].map((word, i) => (
                  <motion.div key={word}
                    ...
                    viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                    className="flex items-center gap-3 py-2.5 px-4 bg-white border border-[#c9a84c]/15 hover:border-[#c9a84c]/50 transition-colors duration-300">
                    <div className="w-px h-5 bg-[#c9a84c]" />
                    <span className="font-serif text-[#0a1628] text-lg italic">{word}</span>
                  </motion.div>
                ))}
              </div>
              <div className="space-y-3">
                ...
                <Link to="/contact"
                  className="flex items-center justify-between px-6 py-4 text-[#0a1628] border border-[#0a1628]/25 hover:border-[#c9a84c] hover:text-[#c9a84c] font-medium text-sm transition-all duration-300 group">
      ...
      {/* ══════════════════════════════════════════════════════════
          04. MUSIC — GOLD / dark (coming soon)
      ══════════════════════════════════════════════════════════ */}
      <section id="music" className="bg-[#060d1a] scroll-mt-20">
        <div className="max-w-6xl mx-auto px-8 lg:px-12 py-24">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 border border-[#c9a84c]/30 flex items-center justify-center text-[#c9a84c]"><Music size={18} /></div>
                <div>
                  <span className="text-[9px] tracking-[0.3em] uppercase text-[#c9a84c]/60 font-medium block">04</span>
              ...
              <p className="font-serif italic text-white/40 text-lg mb-6">A new expression is forming.</p>
              <p className="text-white/55 text-[0.95rem] leading-[1.9] max-w-md">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="bg-[#0a1628] aspect-video flex flex-col items-center justify-center gap-5 relative overflow-hidden border border-[#c9a84c]/10">
                {['♩', '♪', '♫', '♬', '♭'].map((note, i) => (
                  <motion.span key={i} className="absolute font-serif text-[#c9a84c]/20 select-none"
      ...
      <section className="bg-[#112240] py-20 text-center">
        <div className="max-w-2xl mx-auto px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="font-serif font-semibold text-white leading-[1.2] mb-4" style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)' }}>
